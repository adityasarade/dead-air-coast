#!/usr/bin/env python3
"""Render Dead Air's original 16-bar dark heist groove.
...
def render_bass():
    out=[]
    # low minor roots and chromatic approach notes, syncopated pattern per bar
    roots = [31,31,34,29,31,31,36,29,31,31,34,29,31,31,36,29]
    pat = [(0.00,0.42,0),(0.58,0.20,0),(0.92,0.30,7),(1.50,0.20,0),
           (1.75,0.18,12),(2.33,0.32,0),(2.92,0.20,7),(3.25,0.24,0),(3.67,0.22,12)]
    for i in range(N):
        t=i/SR; bar=int(t/(4*BEAT))%16; bt=t%(4*BEAT); v=0.0
        for beat,dur,off in pat:
            ...
            if st <= bt < st+dur*BEAT:
                ...
                v += env(u,dur*BEAT,0.008,0.08) * (0.72*osc(f,u,'saw') + 0.20*osc(f*2,u,'tri') + 0.08*osc(f*1.5,u,'sine'))
        # mild deterministic saturation makes the sub feel driven
        v = math.tanh(v*1.8) * 0.72
        out.append((v*(0.96+0.04*math.sin(t*2)), v*(0.96-0.04*math.sin(t*2))))
    return out

def render_ep():
    out=[]; chords=[[55,58,62],[53,56,60],[58,62,65],[50,53,57]]
    for i in range(N):
        t=i/SR; bar=int(t/(4*BEAT))%16; bt=t%(4*BEAT); v=0.0
        # clipped minor stabs on the offbeat, with occasional two-hit punctuation
        dur=0.62*BEAT
        starts=(0.50,1.50,2.50) if bar%4 != 3 else (0.50,1.50,2.50,3.50)
        bt_beat=bt/BEAT
        hit=min((bt_beat-s for s in starts if s <= bt_beat), default=99)
        if hit < dur/BEAT:
            bt=hit*BEAT
        ...
        v *= (0.86 + 0.14*math.sin(2*math.pi*5.0*t)) * 0.52
...
def render_tension():
    ...
    calls=[(0.75,70,0.18),(1.33,65,0.15),(2.50,68,0.24),(3.33,63,0.18)]
    for i in range(N):
        t=i/SR; bar=int(t/(4*BEAT))%16; bt=t%(4*BEAT); v=0.0
