...
export function judgeEditorSave(signals: EditorGateSignals): EditorGateVerdict {
  const { saved, baseline, current, sawChanges, changeTrackingAvailable } = signals;
  ...
  if (changeTrackingAvailable && comparableImages(baseline, current) && current === baseline)
  ...
  if (changeTrackingAvailable) return { edited: false, reason: "no-tracked-changes" };
