...
Audio now uses the licensed 135 BPM “Chase Pulse” recording described in `music-current-license.md`. One looping player fades between scenes, muting while loading invalidates the pending request, and same-origin tab coordination prevents overlapping playback. Music is user-enabled and off by default.
